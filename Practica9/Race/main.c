...
void *increment(void *arg) { //функция которая будет добавлять +1
    long i, num = *((long *)arg);
    for (i = 0; i < num; i++) {
        val += 1;
    }
...
void *decrement(void *arg) {//функция которая будет забирать -1
    long i, num = *((long *)arg);
    for (i = 0; i < num; i++) {
        val -= 1;
    }
...
int main() {
    pthread_t id1, id2;
    long n;//число иттераций должно быть довольно большим
    printf("\tTest Task1 Conditions\n");
    printf("ENTER n: ");
    scanf("%ld", &n);
    printf("Let do %ld pair operations\n", n);//выводим наши стартовые значение
